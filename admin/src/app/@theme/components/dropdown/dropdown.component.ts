...
import { Placement, PlacementArray, positionElements } from '../utility/positioning';
...
export class DropdownMenu {
  ...
  @ContentChildren(DropdownItem) menuItems: QueryList<DropdownItem>;
 ...
 *
 * This is a simple version of the `DropdownToggle` directive.
...
export class DropdownAnchor {
  ...
  constructor(@Inject(forwardRef(() => Dropdown)) public dropdown, private _elementRef: ElementRef<HTMLElement>) {
 ...
 *
 * You can also use `DropdownAnchor` as an alternative.
 */
@Directive({
  selector: '[thm-dropdown-toggle]',
  host: {
...
export class Dropdown implements AfterContentInit, OnDestroy {
  ...
  */
  ...
  constructor(
      private _changeDetector: ChangeDetectorRef, config: DropdownConfig, @Inject(DOCUMENT) private _document: any,
      private _ngZone: NgZone, private _elementRef: ElementRef<HTMLElement>, private _renderer: Renderer2,
      @Optional() Navbar: ThmNavbar) {
    this.placement = config.placement;
    this.container = config.container;
    this.autoClose = config.autoClose;

    this.display = Navbar ? 'static' : 'dynamic';
